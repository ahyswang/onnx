...
def test_matmul_rhs_group_quant_f16():
    ...
    # 保存模型
    onnx.save(model, "./data.ignore/matmul_rhs_group_quant_with_add_f16.onnx")
    print("模型已保存为 matmul_rhs_group_quant_with_add_f16.onnx")

    # 尝试加载和推导形状
    try:
        loaded_model = onnx.load("./data.ignore/matmul_rhs_group_quant_with_add_f16.onnx")
        ...
        onnx.save_model(inferred_model, "./data.ignore/matmul_rhs_group_quant_with_add_f16_inferred.onnx")
        print("形状推导成功，已保存为 matmul_rhs_group_quant_with_add_f16_inferred.onnx")

        output_info = check_node_output_type_and_shape(
            "./data.ignore/matmul_rhs_group_quant_with_add_inferred_f16.onnx",
            "matmul_rhs_group_quant"
        )
        print(output_info)
        assert output_info["Y"]["type"] == "FLOAT16", "输出类型不匹配，预期为 FLOAT16"
